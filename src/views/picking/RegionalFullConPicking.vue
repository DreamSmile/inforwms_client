  ...
  <div class="all">
    ...
    <div class="box flex_box">
      <div class="row row_small">
        ...
        <select v-model="px" class="two_inp two_select" :disabled="hasCheckQy">
      ...
      <div class="row row_small"><span class="problem three_em">自库位</span><input v-model="showRw.FROMLOC" readonly class="two_inp no_inp" />
        <span class="problem three_em">自箱号</span><input :value="showRw.FROMID || ''" readonly class="two_inp no_inp" />
      </div>
      <div class="row row_small">
        <span class="problem three_em">至库位</span>
        <input :value="showRw.SKU ? 'PICKTO' : ''" readonly class="two_inp no_inp" />
        <span class="problem three_em">至箱号</span>
        <input v-model="zlpn" class="two_inp" @focus="hideVK($event,5)" ref="zlpn" @keyup="zlpn=zlpn.toUpperCase()" />
      </div>
      <div class="detail_page">
        ...
        <div class="no_list page_no_list" v-show="okBoxList.length<1 && !hasHidden && showTable=='okBox'">
          确认货箱后才有数据哦~</div>
        <div class="page_table_content" v-show="okBoxList.length>0 && !hasHidden && showTable=='okBox'">
          <div class="table_row" :style="{backgroundColor:i%2==0?'#FAFAFA':''}" v-for="(list,i) in okBoxList" :key="i">
...
export default {
  ...
  methods: {
    ...
    checkZxh() {
      ...
      this.loading = true;
      let url = `${$apiUrl}jh/qyzxjh/check?storage=STORAGE_ZMC&area=${this.qy}&type=${this.px}&box=${this.showRw.FROMID}&zbox=${this.zlpn}`;
      this.axios
        .get(url)
        .then((res) => {
          this.loading = false;
    ...
    checkQy() {
      ...
      this.loading = true;
      let url = `${$apiUrl}jh/qyzxjh/checkarea?storage=STORAGE_ZMC&area=${this.qy}`;
      this.axios
        .get(url)
        .then((res) => {
          if (res.data.errcode != 0) {
          ...
          let getIfoUrl = `${$apiUrl}jh/qyzxjh/checkmission?storage=STORAGE_ZMC&area=${this.qy}&type=${this.px}`;
          this.axios
            .get(getIfoUrl)
            .then((res) => {
              this.loading = false;
